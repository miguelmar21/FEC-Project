...
var AnswerList = ({ answers }) => {
  var keys = Object.keys(answers).sort((key1, key2) =>
    answers[key1].answerer_name === "Seller"
      ? -1
      : answers[key2].answerer_name === "Seller"
      ? 1
      : answers[key1].helpfulness > answers[key2].helpfulness
      ? -1
      : 1
  );
  ...
  var loadAns = (display) => {
    display === "LOAD MORE ANSWERS"
      ? setAnswers(keys)
      : setAnswers(keys.slice(0, 2));
  };
  ...
  if (!ansIds[0]) {
    ...
    return (
      <div className="al">
        ...
        {numAns > 2 ? <MoreAns numAns={numAns} loadAns={loadAns} /> : ""}
      </div>
    );
  }
};

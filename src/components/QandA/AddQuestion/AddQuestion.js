...
var AddQuestion = () => {
  ...
  const validate = (formValues = values) => {
    ...
    if ("question" in formValues) {
      temp.question = formValues.question
        ? ""
        : "You've not entered your question.";
    }
  ...
  const { values, handleChange, errors, setErrors, reset } = useForm(
    initialValues,
    validate,
    true
  );
  ...
  const handleSubmit = () => {
    ...
    if (noErrors) {
      ...
      reset();
      axios
        .post(`http://localhost:3000/qa/questions/${product_id}/question`, {
  ...
  return (
    <div className="aq">
      <Button text="Add A Question +" variant="outlined" onClick={handleOpen} />
      <Modal
        open={open}
        onClose={handleClose}
      ...
      >
        <TemplateForm>
          ...
          <Stack direction="row" spacing={2}>
            ...
            <Button text="Reset" variant="outlined" onClick={reset} />
          </Stack>
        </TemplateForm>
      </Modal>
    </div>

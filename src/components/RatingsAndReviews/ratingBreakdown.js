...
function RatingBar(props) {
  ...
  return (
    <div>
      <Grid container>
        <Grid item xs={2}>
          <u className="filters" onClick={handleFilterClick}>
            {props.stars} stars
          </u>
        </Grid>
        <Grid item xs={9}>
    ...
    <div>
      ...
      <Grid container>
        <Grid item xs={3}>
          <h1 className="averageRating">
            {props.productContext.meta.averageRating}
          </h1>
        </Grid>
        <Grid item xs={9}>
          <Rating
            size="large"
            value={props.productContext.meta.starRating || 0}
            precision={0.25}
            sx={{ color: "purple", margin: 1.25 }}
            readOnly
          />
        </Grid>
      </Grid>
          ...
          <Grid container>
            ...
            <Grid item xs={9}>
              {props.filters.map((filter, index) => {
                if (filter) {
                  return (
                    <p className="filterText" key={index + 1}>
      ...
      {stars.map((rating) => {
        ...
        var ratingPercentage =
          (numOfRatings / props.productContext.meta.totalRatings) * 100;
        return (
          <RatingBar
            key={rating}
